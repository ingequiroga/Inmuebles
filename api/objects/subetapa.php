...
class SubEtapa{
    ...
    function getxidProc(){
        //echo $this->IdProceso;
         // select all query
         $query = 
                 "SELECT IdTabsubetapa
                 FROM tb_tab_subetapas
                 where IdProceso = ?";
 
         // prepare query statement
         $stmt = $this->conn->prepare($query);
 
         $stmt->bindParam(1, $this->IdProceso);
      ...
      function createSubEtapas(){
        ...
        "INSERT INTO 
            tb_subetapas
            (`Value`,`IdAdquisicion`,`IdTabsubetapa`,`IdProceso`) 
            VALUES (0,:idadquisicion,:idtabsubetapa,:idproceso)";
